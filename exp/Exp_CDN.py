...
from models.CDN import Statistics_prediction
...
class Exp_CDN(Exp_Basic):
    def __init__(self, args):
        super(Exp_CDN, self).__init__(args)
    ...
    def _build_model(self):
        self._get_period()
        print(self.args.period_list)
        print(self.args.period_weight)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if epoch + 1 <= self.station_pretrain_epoch:
                    ...
                    if 'adaptive' in self.args.station_type:
                        outputs = []
                        for k in range(self.args.top_k):
                            if self.args.model == 'AMD':
                                out_k, moe_k = self.backbone_forward(
                                    batch_x[k], batch_y, batch_x_mark, batch_y_mark, k, return_moe_loss=True
                                )
                        ...
                        if self.args.model == 'AMD':
                            outputs, moe_loss = self.backbone_forward(
                                batch_x, batch_y, batch_x_mark, batch_y_mark, k, return_moe_loss=True
                            )
                        else:
                            outputs = self.backbone_forward(batch_x, batch_y, batch_x_mark, batch_y_mark, k)
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pred_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if 'Linear' in self.args.model or 'TST' in self.args.model or self.args.model == 'AMD':
    ...
    def backbone_forward(self, batch_x, batch_y, batch_x_mark, batch_y_mark, k, return_moe_loss=False):
        # decoder input
        ...
        if self.args.use_amp:
            with torch.cuda.amp.autocast():
                if 'Linear' in self.args.model or 'TST' in self.args.model:
                    ...
                    if return_moe_loss:
                        outputs, moe_loss = self.model(batch_x, k, return_moe_loss=True)
                    else:
                        outputs = self.model(batch_x, k)
                else:
                    if self.args.output_attention:
